...
function PixelToggle({ label, value, onToggle }: ToggleRowProps) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-white/10">
      <span className="font-terminal text-[20px] text-white">{label}</span>
      <button
        onClick={() => onToggle(!value)}
        className={[
          'relative w-12 h-6 border-2 border-white shadow-[2px_2px_0px_#000] transition-none cursor-pointer',
          value ? 'bg-[#22c55e]' : 'bg-[#1e2433]',
        ].join(' ')}
        role="switch"
        aria-checked={value}
      >
        <span
          className={[
            'absolute top-0.5 w-4 h-4 bg-white shadow-[1px_1px_0px_#000] transition-none',
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center">
      <div className="w-full max-w-[480px] bg-[#0a0a0f] border-t-4 border-white shadow-[0_-4px_0_#000] p-6 pb-8 max-h-[80vh] overflow-y-auto">
        {/* Title */}
        <div className="font-pixel text-[12px] text-white mb-6">SETTINGS</div>
